...
B<psych> [B<-bdi>] [B<-a> I<number>] [B<-p> I<number>] [B<-e> I<code> | I<file>]
...
comments in the code beginning with the character C<#>.
...
will not begin until EOF is reached).  If the B<-e> switch is used, the given
...
Turn off input buffering for the C<,> command.  Note that, if input is from a
terminal, the EOF condition is not recognized while this is in effect.

=item B<-d>

Make C<.> output values as numbers rather than characters
...
Print the values of the first I<number> elements of the data array as numbers
...
respectively) may be reconfigured at compile time; run C<psych -V> to confirm
these values.
